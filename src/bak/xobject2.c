...
x_type_ref              (xType          type)
{
    ...
    x_rwlock_wlock (&rw_type);
    info = x_type_info(type);
    if (info)
        ref = x_atomic_int_inc (&info->ref);
    else
        ref = 0;
    x_rwlock_unwlock (&rw_type);
...
x_type_parent           (xType          type)
{
    ...
    x_rwlock_rlock (&rw_type);
    info = x_type_info(type);
    if (info)
        type = info->supers[1];
    else
        type = X_TYPE_VOID;
    x_rwlock_unrlock (&rw_type);
...
x_type_fundamental      (xType          type)
{
    ...
    info = x_type_info(type);
    if (info)
        type = info->supers[info->n_supers];
    else
        type = X_TYPE_VOID;
    x_rwlock_unrlock (&rw_type);
...
x_instance_private      (xInstance      *instance)
{
    ...
    info = x_type_info (X_INSTANCE_TYPE(instance));
    if (info) {
        xsize private_offset = klass->instance_size;
        if (info->n_supers > 1) {
            xTypeInfo   *pinfo = x_type_info (info->supers[1]);
            if (pinfo && pinfo->klass)
                private_offset += X_CLASS (pinfo->klass)->private_offset;
        }
        prv_ptr = X_STRUCT_MEMBER_P(instance, private_offset);
    } else {
        prv_ptr = NULL;
    }
    x_rwlock_unrlock (&rw_type);
