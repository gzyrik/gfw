...
#define bigEndian rdp::bigEndian
#define AssignUWord16ToBuffer rdp::wordToBuffer
#define BufferToUWord16 rdp::bufferToWord
#define AssignUWord32ToBuffer rdp::longToBuffer
namespace fec {
// Minimum RTP header size in bytes.
const uint8_t kRtpHeaderSize = 0;//12;
...
int32_t ForwardErrorCorrection::GenerateFEC(
  ...
  if (mediaPacketList.empty()) {
    LOG_E("%s media packet list is empty", __FUNCTION__);
    return -1;
  }
  if (!fecPacketList->empty()) {
    LOG_E("%s FEC packet list is not empty", __FUNCTION__);
    return -1;
  }
  const uint16_t numMediaPackets = (uint16_t)mediaPacketList.size();
  ...
  if (numMediaPackets > kMaxMediaPackets) {
    LOG_E("%s can only protect %d media packets per frame; %d requested",
  ...
  if (numImportantPackets > numMediaPackets) {
    LOG_E("Number of important packets (%d) greater than number of media "
      "packets (%d)", numImportantPackets, numMediaPackets);
    return -1;
  }
  if (numImportantPackets < 0) {
    LOG_E("Number of important packets (%d) less than zero",
      numImportantPackets);
    return -1;
  }
  ...
  while (mediaListIt != mediaPacketList.end()) {
    ...
    if (mediaPacket->length < kRtpHeaderSize) {
      LOG_E("%s media packet (%d bytes) is smaller than RTP header",
    ...
    if (mediaPacket->length + PacketOverhead() + kTransportOverhead >
      IP_PACKET_SIZE) {
      LOG_E("%s media packet (%d bytes) with overhead is larger than MTU(%d)",
  ...
  //LOG_D("media packets:%d, FEC packets:%d", numMediaPackets, numFecPackets);
  ...
  GenerateFecUlpHeaders(mediaPacketList, packetMask, numFecPackets, sequenceNumberBase);
    ...
    while (mediaListIt != mediaPacketList.end()) {
      ...
      if (packetMask[pktMaskIdx] & (1 << (7 - mediaPktIdx))) {
        ...
        if (_generatedFecPackets[i].length == 0) {
          // Copy the first 2 bytes of the RTP header. V,P,X,M,PT
          memcpy(_generatedFecPackets[i].data, mediaPacket->data, 2);
          // Copy the 5th to 8th bytes of the RTP header. RTPTimestamp
...
void ForwardErrorCorrection::GenerateFecUlpHeaders(
  ...
  for (uint32_t i = 0; i < numFecPackets; i++) {
    ...
    if (kRtpHeaderSize != 0)
      ...
      AssignUWord16ToBuffer(&_generatedFecPackets[i].data[2],sequenceNumberBase);
...
void ForwardErrorCorrection::InsertFECPacket(
  ...
  if (fecPacket->protectedPktList.empty()) {
    ...
    LOG_W("FEC packet %u has an all-zero packet mask.",
...
void ForwardErrorCorrection::InsertPackets(
  ...
  // get last sequnce to discard packet
  int32_t lastMediaSeqNo = -1;
  int32_t lastFecSeqNo = -1;
  ...
  DiscardOldPackets(recoveredPacketList, lastMediaSeqNo);
  DiscardOldFecPackets(&_fecPacketList,lastFecSeqNo);
}
...
int ForwardErrorCorrection::XorPackets(const Packet* src_packet,
  ...
  if (src_packet->length >= IP_PACKET_SIZE)
  {
    ASSERT(false);
    return -1;
  }
...
int ForwardErrorCorrection::RecoverPacket(
  ...
  while (protected_it != fecPacket->protectedPktList.end()) {
    if ((*protected_it)->pkt == NULL) {
      ...
      if (XorPackets((*protected_it)->pkt, recPacketToInsert) !=0)
        return -1;
    }
    ++protected_it;
  }
...
int ForwardErrorCorrection::AttemptRecover(
  RecoveredPacketList* recoveredPacketList) {
  FecPacketList::iterator fecPacketListIt = _fecPacketList.begin();
  while (fecPacketListIt != _fecPacketList.end()) {
    ...
    if (packets_missing == 1) {
      ...
      if (RecoverPacket(*fecPacketListIt, packetToInsert) != 0)
      {
...
void ForwardErrorCorrection::DiscardOldPackets(
  RecoveredPacketList* recoveredPacketList,
  int32_t lastSeqNo) {
  while (recoveredPacketList->size() > kMaxRecoverPackets) {
  ...
  assert(recoveredPacketList->size() <= kMaxRecoverPackets);
}
...
int32_t ForwardErrorCorrection::DecodeFEC(
  ...
  // error?
  if (recoveredPacketList->size() == kMaxRecoverPackets) {
    const uint16_t seq_num_diff =
      receivedPacketList->front()->seqNum - recoveredPacketList->back()->seqNum;
    if (seq_num_diff > 0x01ff && seq_num_diff < 0xfc00) {
...
uint16_t ForwardErrorCorrection::LatestSequenceNumber(uint16_t first,
  uint16_t second) {
  bool wrap = (first < 0x0fff && second > 0xf000) ||
    (first > 0xf000 && second < 0x0fff);
